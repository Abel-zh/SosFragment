...
public class AsyncHttpRequest implements Runnable
{
	...
	private void makeRequestWithRetries() throws ConnectException
	{
		...
		while (retry)
		{
			...
			{
				LogUtil.w(TAGLOG.TAG_HTTP, e.toString());
				
				if (responseHandler != null)
				{
					responseHandler.sendFailureMessage(e, "网络连接失败！");
				}
				return;
			...
			} catch (SocketTimeoutException e)
			{
				...
				if (responseHandler != null)
				{
					responseHandler.sendFailureMessage(e, "网络连接超时！");
				}
				return;
			} catch (IOException e)
			{
				...
				LogUtil.w(TAGLOG.TAG_HTTP, e.toString() + "   retry-->" + retry);
