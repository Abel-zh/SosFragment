...
import com.naicha.module.UserCropImageActivity;
...
public class CropImage {
	...
	public static final File FILE_LOCAL = new File(FILE_SDCARD, "sosino_user_img");
				...
				mHandler.post(new Runnable() {
					public void run() {
						try {
							Matrix m = new Matrix();
							m.setRotate(degrees);
							Bitmap tb = Bitmap.createBitmap(mBitmap, 0, 0, mBitmap.getWidth(), mBitmap.getHeight(), m,
									false);
							mBitmap = tb;
	...
	private Bitmap onSaveClicked(Bitmap bm) {
		...
		int width = 120;// dr.width(); // modify by yc
		int height = 120;// dr.height();
	...
	class BackgroundJob implements Runnable {
		...
		public void run() {
			...
			mHandler.post(new Runnable() {
				public void run() {
					try {
						mHandler.sendMessage(mHandler.obtainMessage(UserCropImageActivity.SHOW_PROGRESS));
					} catch (Exception e) {
						// TODO: handle exception
					}
			...
			try {
				...
				mHandler.sendMessage(mHandler.obtainMessage(UserCropImageActivity.REMOVE_PROGRESS));
			}
		}
	}
}
